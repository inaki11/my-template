...
def main():
    ...
    parser.add_argument("--total-runs", required=True, type=int, help="Número total de ejecuciones")
    parser.add_argument("--processes", required=True, type=int, help="Número de procesos paralelos")
    ...
    result = subprocess.run(
        [sys.executable, "-m", "wandb", "sweep", "--project", PROJECT, "--entity", ENTITY, sweep_config],
    ...
    if not match:
        print("❌ No se pudo extraer el sweep ID. Verifica que el sweep fue creado correctamente.")
        print(output)
        sys.exit(1)
